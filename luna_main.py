...
def plot_cluster(image_time, cluster_vector, length, time):
    plt.subplot(1, 2, 1)
    plt.imshow(image_time, extent=[0, length, 0, time])
    plt.xlabel('L [mm]')
    plt.ylabel('Timestamp [-]')
...
def demo():
    ...
    plot_cluster(time_derivative_image_right, k_means_cluster, delta_length_right, delta_time_right)

    mean_shift_cluster = mean_shift(panel)

    plot_cluster(time_derivative_image_right, mean_shift_cluster, delta_length_right, delta_time_right)
